...
export function activate(context: vscode.ExtensionContext) {
  ...
  async function compile() {
    ...
    if (!cwd) throw new Error('No workspace found')

    const inputFiles = `${cwd}/**/*.c`
    const outputFile = `${configuration.tmpFolder}/a.out`

    const files = await glob(inputFiles)
    if (files.length === 0) throw new Error('No .c files found')
    const filePaths = files.join(' ')

    await execAsync(
      `cc ${filePaths} -o ${outputFile} -fno-diagnostics-fixit-info -fno-caret-diagnostics -fno-color-diagnostics`
    )
  ...
  const compileC = vscode.commands.registerCommand(
    'easy-c.compile-c',
    async () => {
      try {
